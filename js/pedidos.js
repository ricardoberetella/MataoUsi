...
document.addEventListener("DOMContentLoaded", () => {
  ...
  document.addEventListener("click", async (e) => {
    ...
    if (btn.matches(".btn-excluir-pedido")) {
      ...
      if (confirm(`Tem certeza que deseja excluir o pedido ${numero || id}?`)) {
  ...
  async function carregarPedidos() {
    ...
    let query = supabase
      .from("pedidos")
      .select("id, cliente_id, data_pedido, tipo_pedido, total, numero_pedido, pc_oc")
      .order("id", { ascending: false });

    const numero = filtroNumero.value.trim();
    if (numero) {
      query = query.ilike("numero_pedido", `%${numero}%`);
    }

    const { data: pedidos, error } = await query;

    if (error) {
      tbody.innerHTML = `<tr><td colspan="7">Erro ao carregar pedidos: ${error.message}</td></tr>`;
      return;
    }
    ...
    for (const p of pedidos) {
      ...
      tr.innerHTML = `
        ...
        <td>
          <button class="btn btn-secondary btn-abrir-pedido" type="button" data-id="${p.id}">
            Abrir
          </button>
          <button class="btn btn-ghost btn-excluir-pedido" type="button" data-id="${p.id}" data-numero="${p.numero_pedido}">
            Excluir
          </button>
        </td>
      `;
...
async function carregarNomesClientes(listaIds) {
  const mapa = new Map();
  const idsUnicos = [...new Set(listaIds.filter(id => id != null))];
